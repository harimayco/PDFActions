...
self.onmessage = ({ data }) => {
  ...
  const stripMetadata = data.stripMetadata !== false;
  GSPS2PDF(
    {
      psDataURL: data.pdfUrl,
      quality: pdfQuality,
    ...
    Module = {
      preRun: [function () {
        ...
        if (FS) {
          ...
          // Remove all document metadata, author, title, producer, and XMP XML
          if (dataStruct.stripMetadata !== false) {
            const pdfmark = `[ /Title ()
  /Author ()
  /Subject ()
  /Keywords ()
...
`;
            FS.writeFile('pdfmark.ps', pdfmark);
          }
        }
      }],
      postRun: [function () {
      ...
      arguments: [
        ...
        `-dMonoImageResolution=${qualityConfig.monoResolution || qualityConfig.resolution}`,
        ...
        'input.pdf',
        ...(dataStruct.stripMetadata !== false ? ['pdfmark.ps'] : [])
      ],
      print: function (text) {
...
function getQualitySettings(qualityCode, customResolution) {
  ...
  return {
    ...
    monoResolution: String(Math.max(300, Math.round(targetResolution * 1.5))),
  };
}